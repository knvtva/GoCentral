...
func (service BattleCreateService) Handle(data string, database *mongo.Database, client *nex.Client) (string, error) {
	...
	battlesCollection := database.Collection("battles") 
	...
	BattleID := rand.Intn(100000000)
	battlesDocument := bson.M{
		"id": req.Instrument,
		"type": req.Type,
		"title": req.Name,
		...
		"owner": user.Username,
		"art_url": "",
		"time_end_val": req.TimeEndVal,
		"time_end_units": req.TimeEndUnits,
    }
	...
	for i := 0; i <= 99; i++ {
		...
		if songID != 0 {
			battlesDocument[fieldName] = songID
		}
	}
	
	_, err = battlesCollection.InsertOne(context.TODO(), battlesDocument)

	res := []BattleCreateResponse{{0, BattleID}}
